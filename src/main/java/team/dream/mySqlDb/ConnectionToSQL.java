...
public class ConnectionToSQL {
    ...
    private final String sshPassword = properties.getProperty("ssh.password");
    ...
    private void connectToSSH(){
        ...
        try {
            ...
            IO.println("Connected to " + sshHost);
            ssh.authPassword(sshUser,sshPassword);
            IO.println("Auth success");

            Parameters parameters = new Parameters(localhost,forwardedLocalPort, localhost,dbPort);

            localSocket = new ServerSocket();
            ssh.newLocalPortForwarder(parameters,localSocket).listen();
